...
function load_dataset(args...; dataset=OMNI_HRO_1MIN, name=dataset.name, products=dataset.parameters)
    vars = mapreduce(merge, products) do p
        TimeArray(replace_fillval_by_nan!(get_data("cda/$name/$p", args...)))
    end
end

timespan = ["2015-03-15", "2015-03-22"] # 7 days
omni_hro_ds = load_dataset(timespan; dataset=OMNI_HRO_1MIN)


OMNI2_H0_MRG1HR_ds = load_dataset(timespan; dataset=OMNI2_H0_MRG1HR)
```
