...
$$
\sigma^2=\frac{1}{M} \sum_{m=1}^M\left|\left(\bf{B}^{(m)}-\langle\bf{B}\rangle\right) \cdot \hat{\bf{n}}\right|^2
$$ {#eq-Chen84}
...
$$
\left(\bf{B}^{(m)} - \langle \bf{B} \rangle\right) \cdot \hat{\bf{n}} = \sum_{\mu=1}^3 \left(B_\mu^{(m)} - \langle B_\mu \rangle\right) n_\mu,
$$
...
$$
L(\hat{\bf{n}}, \lambda) = \sigma^2 - \lambda\left(\sum_{\mu=1}^3 n_\mu^2 - 1\right) = \sum_{\mu,\nu=1}^3 n_\mu M_{\mu\nu}^B n_\nu - \lambda\left(\sum_{\mu=1}^3 n_\mu^2 - 1\right).
$$
...
This is exactly Equation (8.7): $\sum_{\nu=1}^3 M_{\mu \nu}^B n_\nu = \lambda n_\mu$. The equation is an eigenvalue problem for the symmetric $3\times 3$ matrix $M_{\mu\nu}^B$. The eigenvalues $\lambda$ are real, and the corresponding eigenvectors $\hat{\bf{n}}$ are orthogonal.


<!-- 
The minimum variance technique is designed to deal with the situation where some or all of the non-ideal effects mentioned above, except a systematic temporal change in the normal direction, $\hat{\bf{n}}$, are present. As the estimate of $\hat{\bf{n}}$, the method identifies that direction in space along which the field-component set $\left\{\bf{B}^{(m)} \cdot \hat{\bf{n}}\right\}(m=1,2,3 \ldots M)$ has minimum variance. In other words, $\hat{\bf{n}}$ is determined by minimisation of

$$
\sigma^2=\frac{1}{M} \sum_{m=1}^M\left|\left(\bf{B}^{(m)}-\langle\bf{B}\rangle\right) \cdot \hat{\bf{n}}\right|^2
$$

where the average $\langle\bf{B}\rangle$ is defined by

$$
\langle\bf{B}\rangle \equiv \frac{1}{M} \sum_{m=1}^M \bf{B}^{(m)}
$$
...
$$
\begin{aligned}
\frac{\partial}{\partial n_X}\left(\sigma^2-\lambda\left(|\hat{\bf{n}}|^2-1\right)\right) & =0 \\
\frac{\partial}{\partial n_Y}\left(\sigma^2-\lambda\left(|\hat{\bf{n}}|^2-1\right)\right) & =0 \\
\frac{\partial}{\partial n_Z}\left(\sigma^2-\lambda\left(|\hat{\bf{n}}|^2-1\right)\right) & =0
\end{aligned}
$$

where $\sigma^2$ is given by equation 8.4 and $\hat{\bf{n}}$ is represented in terms of its three components ( $n_X, n_Y, n_Z$ ) along the cartesian coordinate system $X, Y, Z$ (e.g., GSE or GSM) in which the field data $\left\{\bf{B}^{(m)}\right\}$ are given. When the differentiations in equation 8.6 have been performed, the resulting set of three equations can be written in matrix form as
...
## This document was built using these direct dependencies
