...
```{julia}
begin
    E = tclip(thm_efw, trange_wb) |> standardize |> timeshift
    E_clean = replace_outliers(E; window=128)
    E_sm = tfilter(E, 64u"Hz")

    tvars = [E, E_clean, E_sm]
    f = Figure()
    tplot(f[1, 1], tvars)
    fa2 = tplot(f[1, 2], tvars)
...
begin
    ...
    E_clean_sm = tfilter(E_clean, 64u"Hz")

    S = Poynting_vector(B, E)
    S_sm = Poynting_vector(B_sm, E_clean_sm)

    f = Figure()
    tplot(f[1, 1], [thm_scw_dsl, thm_efw])
    tplot(f[1, 2], [B, E, S])
    tplot(f[1, 3], [B_sm, E_clean_sm, S_sm])
    f
end
```
...
"""
function calculate_poynting_flux_time(E, B, fsmin::Float64)
    # Sampling parameters
    fs = 1 / scwtres         # sampling frequency [Hz]
    fnyquist = samplingrate(E) / 2        # Nyquist frequency [Hz]
