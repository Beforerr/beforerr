...
theta_test = [200, 1860]
model_values = model(theta_test, x)
...
stairs!(ax, x, model_values, label="Model (A=$(theta_test[1]), B=$(theta_test[2]))")
f
```
...
prior(theta) = prior(theta[1], theta[2])

# Test the prior function with A=200, B=1860
theta_test = [200, 1860]
prior_value = prior(theta_test)
...
function like(theta, data, x; xpos=10, sigma=30.0)
    # Calculate the model prediction
    m = model(theta, x, xpos=xpos)
...
theta_test = [200, 1860]
likelihood_value = like(theta_test, data, x)
...
function like_times_prior(theta, data, x; xpos=10, sigma=30.0)
    return like(theta, data, x, xpos=xpos, sigma=sigma) * prior(theta)
end

# Test the function with A=200, B=1860
theta_test = [200, 1860]
posterior_numerator = like_times_prior(theta_test, data, x)
