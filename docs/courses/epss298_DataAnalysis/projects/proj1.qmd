...
f = Figure()
markersize = 4
scatterlines(f[1, 1], data.time, data.flux; markersize)
f
```
...
orbit = KeplerianOrbit(period=per_i, t0=t0_i, a=a_i, b=b_i, ecc=ecc_i)
ld = QuadLimbDark(u_i)
fluxes = @. ld(orbit, data.time, rp_i)
...
Here we use [Turing.jl](https://github.com/TuringLang/Turing.jl) to sample the posterior distribution instead.
...
```{julia}
@model function transit_model(time, flux; ld=QuadLimbDark(u_i), period=per_i, error=1e-4, a=a_i)
    t0 ~ Uniform(1.212, 1.362)
    rp ~ Uniform(0.01, 0.10)
    b ~ Uniform(0.0, 0.99)
    orbit = KeplerianOrbit(; period, t0, b, a)
    model_flux = @. ld(orbit, time, rp)
    return flux ~ MvNormal(model_flux, error)
end

model = transit_model(data.time, data.flux)
chain = sample(model, Emcee(16), 10000; progress=false)
```
...
inc_fit = inc(params, b_fit)
params_fit = set_params!(params; t0=t0_fit, rp=rp_fit, inc=inc_fit)
model_flux = light_curve(trans_model, params_fit)
...
```{julia}
@model function transit_model_two_bodies(time, flux, params1, trans_model1, params2, trans_model2; error=1e-4)
    # Priors for planet
    t0_1 ~ Uniform(1.212, 1.362)
    rp_1 ~ Uniform(0.01, 0.10)
    b_1 ~ Uniform(0.0, 0.99)
    # Priors for moon (or second planet)
    t0_2 ~ Uniform(1.212, 1.362)
    rp_2 ~ Uniform(0.01, 0.10)
    b_2 ~ Uniform(0.0, 0.99)

    params1 = set_params!(params1; t0=t0_1, rp=rp_1, inc=inc(params1, b_1))
    params2 = set_params!(params2; t0=t0_2, rp=rp_2, inc=inc(params2, b_2))
    model_flux1 = light_curve(trans_model1, params1)
    model_flux2 = light_curve(trans_model2, params2)
    combined_flux = model_flux1 .* model_flux2
    return flux ~ MvNormal(combined_flux, error)
end
```
...
```{julia}
params1 = TransitParams(; rp=rp_i, t0=t0_i, per=3.0, a=a_i, u=u_i, inc=inc_i)
params2 = TransitParams(; rp=rp_i, t0=t0_i, per=3.0, a=a_i, u=u_i, inc=inc_i)
trans_model1 = TransitModel(params1, data.time)
trans_model2 = TransitModel(params2, data.time)
model = transit_model_two_bodies(data.time, data.flux, params1, trans_model1, params2, trans_model2)
chain = sample(model, Emcee(100), 10000; progress=false)
```
